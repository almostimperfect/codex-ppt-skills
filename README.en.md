...
Convert picture-only or image-based PPT decks into editable PPTX files where the text can be revised.
...
- Rewrote `image-ppt-to-editable` skill instructions in Chinese-first wording while preserving key technical terms such as `layout JSON`, `textless background`, and `editable text boxes`.
- Rewrote `image-based-ppt-generator` skill instructions in Chinese-first wording and clarified that editable-text requests should route to `image-ppt-to-editable`.
- Made Chinese the default repository README and moved English documentation to `README.en.md`.

### v1.4

- Expanded `image-ppt-to-editable` trigger wording for requests such as editable version, text-editable PPT, 可编辑版本, and 文字可编辑版本.
- Updated user-facing wording to say editable/text-editable.
